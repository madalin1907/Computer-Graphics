...
// ================================================
// | Laboratorul VIII - 08_05_spirala.cpp         |
// ================================================
// 
...
void CreateVBO(void)
{
	...
	{
		vf_coord[i] = glm::vec4(a * (cos(i) + i * sin(i)),
							    a * (sin(i) + i * cos(i)),
							    0.0f, 1.0f);
	};
	// Culorile varfurilor
	...
	for (int i = 0; i < NR_POINTS; i++)
	{
		vf_col[i] = glm::vec4(abs(0.5 * i / NR_POINTS + cos(0.1 * i)),
							  abs(0.4 * i / NR_POINTS + sin(0.01 * i)),
							  abs(0.5 * i / NR_POINTS - sin(0.1 * i)), 1.0f);
	};
	// Indicii
...
void RenderFunction(void)
{
	...
	glLineWidth(0.1f);
	glDrawElements(GL_LINE_STRIP, NR_POINTS, GL_UNSIGNED_INT, (void*)(0));
	// Desenarea punctelor
	codCol = 0;
	glUniform1i(codColLocation, codCol);
	glPointSize(5.0f);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Spirala");		//	Creeaza fereastra de vizualizare, indicand numele acesteia;
